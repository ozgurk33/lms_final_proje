...
const InstructorDashboard = ({ navigation }) => {
    ...
    const renderCourseCard = ({ item }) => (
        <TouchableOpacity
            ...
            accessibilityLabel={`${item.title} kursu. ${item._count?.enrollments || 0} öğrenci`}
            accessibilityHint="Kursu düzenlemek için dokunun">
            <Text style={styles.courseTitle} accessibilityRole="header">
                {item.title}
            </Text>
            <Text style={styles.courseDescription} numberOfLines={2}>
                {item.description}
            </Text>
            <View style={styles.courseFooter} accessible={false}>
